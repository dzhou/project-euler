...
def problem11():
    ...
    line_list = []
    # Add vertical/horiz lines
    for i in range(0, len(grid[0])):
        line_list.append([grid[j][i] for j in range(0, len(grid))])
        line_list.append([grid[i][j] for j in range(0, len(grid))])
    ...
    for i in range(0, len(grid)):
        ...
        for j in range(0, i+1):
            line_a.append(grid[j][i-j])
            line_b.append(grid[max_x-(j)][max_y-(i-j)])
        line_list.extend([line_a,line_b])

    for i in range(0, max_x):
        line_a = []
        line_b = []
        for j in range(0, len(grid)-i):
            line_a.append(grid[j][j+i])
            line_b.append(grid[j+i][j])
        line_list.extend([line_a,line_b])
...
def problem13():
    ...
    50-digit numbers. 
    File: files/p13_numbers
    """
    f = open('files/p13_numbers', 'rb')
    data = f.readlines()
    f.close()

    t = sum([int(x) for x in data])
    return int(str(t)[:10])
...
def problem14():
    ...
    """
    def chaining(n, chain_map):
        if n in chain_map:
            return chain_map[n]
        else:
            if n % 2 == 0:
                m = chaining(n/2, chain_map) + 1
            else:
                m = chaining(n*3+1, chain_map) + 1
...
def problem15():
    ...
    #return factorial(20*2) / (factorial(20) ** 2)
    size = 21
    ...
    while layer < size:
        for i in range(layer, size):
            grid[layer][i] = grid[layer-1][i] + grid[layer][i-1]
            grid[i][layer] = grid[i][layer-1] + grid[i-1][layer]
        layer += 1
...
def problem17():
    ...
    for i in range(1, 1000):
        ...
        s = ""
        if len(n) == 3:
            s += num_map[int(n[0])] + 'hundredand'
            if i % 100 == 0:
                s = s[:-3]
            n = n[1:]

        if len(n) == 2 and int(n) < 20:
            s += num_map[int(n)]
            n = ""
        elif len(n) >= 2:
            s += num_map[int(n[0])*10]
            n = n[1]
